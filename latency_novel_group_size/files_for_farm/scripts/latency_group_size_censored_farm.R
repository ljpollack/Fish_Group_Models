library(package = "brms", lib.loc = "/home/mjculsha/RPackages/R3.4.4")
library(tidyverse)

# read data
d <- read_csv("data/cleaned_data_farm.csv")
d$group_ID <- as.factor(d$group_ID)
d$tank <- as.factor(d$tank)

d <- d %>% 
  mutate(
    is_censored = case_when(latency == 300 ~ 1,
                            TRUE ~ 0)
  )


# fit model
model_1 <- brm(data = d, family = negbinomial, 
               latency | cens(is_censored) ~ 1 + treatment + trial +
                 (1 + treatment + trial | group_ID) +
                 (1 + treatment + trial | tank),
               prior = c(set_prior("normal(0, 1)", class = "Intercept"),
                         set_prior("normal(0, 1)", class = "b"),
                         set_prior("cauchy(0, 2)", class = "sd"),
                         set_prior("lkj(4)", class = "cor")),
               iter = 5000, warmup = 1000, chains = 3, cores = future::availableCores(),
               control = list(adapt_delta = 0.98, max_treedepth = 15), save_model = "fit_models/stan_model_4")

# save model
saveRDS(model_1, "fit_models/model_4_fit.rds")
